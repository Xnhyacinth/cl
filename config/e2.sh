...
#         r2=8
#     fi
#     bash config/run1.sh 2 8,9 t5-large vida 1 constant 1e-4 0 all 0 8 -1 $item $r2 0.33
# done
ranks="4"
for item in $ranks; do
    echo "Item: $item"
    ...
    bash config/run4.sh 2 8,9 t5-large vida 1 constant 1e-4 0 all 0 8 -1 $item $r2 0.33
done
...
# nohup bash config/e2.sh > logs/d.log 2>&1 &
