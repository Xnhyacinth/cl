...
def uninject_trainable_vida(model: nn.Module, target_replace_module: List[str] = ["CrossAttention", "Attention"]):
    """
    Uninject ViDA from the model, restoring the original nn.Linear modules.
    """

    for _module in model.modules():
        if _module.__class__.__name__ in target_replace_module:
            for name, _child_module in _module.named_modules():
                # Check if the child module is an instance of ViDAInjectedLinear
                if isinstance(_child_module, ViDAInjectedLinear):
                    # Extract the original linear_vida layer
                    original_linear = _child_module.linear_vida

                    # Replace ViDAInjectedLinear with the original nn.Linear module
